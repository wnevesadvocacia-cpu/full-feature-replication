CREATE OR REPLACE FUNCTION public.notify_tasks_due_soon(_days_ahead integer DEFAULT 2)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    _label := CASE WHEN _days_ahead = 2 THEN '48 horas' ELSE _days_ahead || ' dias' END;
    _subject := 'WnevesBox — Tarefa vence em ' || _label;
    _msg_id := gen_random_uuid()::text;

    _html :=
      '<div style="font-family:Arial,sans-serif;background:#fff;padding:24px;color:#222">' ||
      '<h2 style="margin:0 0 12px">⏰ Tarefa vence em ' || _label || '</h2>' ||
...
SELECT cron.schedule(
  ...
  '0 12 * * *',
  $$ SELECT public.notify_tasks_due_soon(2); $$
);