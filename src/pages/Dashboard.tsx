...
export default function Dashboard() {
  const { data: processes = [], isLoading: lp } = useProcesses();
  ...
  const activeProcesses = (processes as any[]).filter(p => p.status !== 'concluido').length;
  ...
  const totalBilled = (invoices as any[]).filter(i => i.status === 'pago').reduce((sum, i) => sum + Number(i.amount), 0);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      {(processes as any[]).length === 0 && (tasks as any[]).length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-card rounded-lg p-5 shadow-card">
            ...
            <div className="space-y-3">
              {(processes as any[]).slice(0, 5).map((p) => (
          ...
          <div className="bg-card rounded-lg p-5 shadow-card">
            ...
            <div className="space-y-3">
              {(tasks as any[]).filter(t => !t.completed).slice(0, 5).map((t) => (
                <div key={t.id} className="flex items-center justify-between text-sm p-2 rounded hover:bg-muted/50 transition-colors">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${t.priority === 'alta' ? 'bg-destructive/10 text-destructive' : t.priority === 'media' ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'}`}>
                    {t.priority}
                  </span>
                </div>
              ))}
