import { AlertTriangle, AlarmClock, CalendarClock, ShieldAlert, CheckCircle2, FileSignature } from 'lucide-react';
...
export function DeadlineBadge({ deadline, receivedAtISO }: Props) {
  // Política de segurança jurídica (Sprint Crítico — decisão final):
  // confiança < 0.8, ambígua ou auto_baixa → NÃO mostrar nenhum valor de
  // prazo (data, contagem, peça presumida). Apenas o banner vermelho de
  // revisão urgente. Mostrar valor presumido = risco de malpractice.
  const isUnsafe =
    deadline.confianca < 0.8 ||
    deadline.classificacaoStatus === 'ambigua_urgente' ||
    deadline.classificacaoStatus === 'auto_baixa';

  if (isUnsafe) {
    return (
      <div
        role="alert"
        className="w-full inline-flex items-start gap-2 px-3 py-2 rounded-md border-2 border-destructive bg-destructive/10 text-destructive"
      >
        <ShieldAlert className="h-5 w-5 mt-0.5 shrink-0 animate-pulse" />
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold uppercase tracking-wide">
            ⚠ Prazo não identificado — revise urgente
          </div>
          <div className="text-xs mt-0.5 text-destructive/90 leading-snug">
            A classificação automática não atingiu confiança suficiente para fixar prazo.
            O advogado responsável DEVE conferir o teor da decisão e cadastrar o prazo manualmente
  ...
  return (
    <div className="inline-flex flex-wrap items-center gap-1.5">
      ...
      {/* Peça sugerida — só renderiza para classificações confiáveis */}
      <TooltipProvider delayDuration={150}>
        <Tooltip>
          <TooltipTrigger asChild>
            <span
